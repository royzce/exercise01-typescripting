...
function saveData(city, country, population) {
    ...
    var oldMergedCityInfo = JSON.parse(localStorage.getItem("mergedCityInfo") || "{}");
    oldMergedCityInfo.push(newMergedCityInfo);
    localStorage.setItem("mergedCityInfo", JSON.stringify(oldMergedCityInfo));
    showTableData();
}
function showTableData() {
    document.getElementById("list-of-cities").innerHTML = `<li class="list-group-item active">List of Cities</li>`;
    if (localStorage.getItem("mergedCityInfo") != null) {
...
function showSearchResult(searchString) {
    document.getElementById("list-of-cities").innerHTML = `<li class="list-group-item active">List of Cities</li>`;
    if (!searchString) {
        return;
    }
